        ...
        if 'body' in child.tag:
            for children in child:
                if children.tag == "{http://www.tei-c.org/ns/1.0}p" or \
                    ...
                    add_xml_content(children, file, 'german')
                    for more_children in children:
                        add_xml_content(more_children, file, 'german')
                        for even_more_children in more_children:
                            add_xml_content(even_more_children, file, 'german')
                            for wow_more_children in even_more_children:
                                add_xml_content(wow_more_children, file, 'german')
                                for much_more_children in wow_more_children:
                                    add_xml_content(much_more_children, file, 'german')
