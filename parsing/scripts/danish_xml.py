...
def scan_for_subtags(root, file):
    if root.tag == "{http://www.tei-c.org/ns/1.0}pb":
        add_xml_content(root, file, "danish")
            ...
            for child in kid:
                ...
                if "div" in child.tag:
                    for children in child:
                        ...
                        if children.tag == "{http://www.tei-c.org/ns/1.0}p":
                            add_xml_content(children, file, "danish")
                            for morechildren in children:
...
def get_poetry(root, file):
    for child in root:
        if child.tag == "{http://www.tei-c.org/ns/1.0}l":
            add_xml_content(child, file, "danish")
            for children in child:
...
def main():
    ...
    if args.o is None:
        fail("Please specify output (-o) directory.")
    if args.csv is None:
        fail("Please specify csv (-csv) directory.")
